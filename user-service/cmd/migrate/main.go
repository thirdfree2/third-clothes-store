...
func migrateDown(db *gorm.DB) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Exec("DELETE FROM user_schema_migrations WHERE version = ?", version).Error; err != nil {
			return err
		}
...
func applyUp(db *gorm.DB, file migrationFile) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		return tx.Exec("INSERT INTO user_schema_migrations (version) VALUES (?)", file.Version).Error
	})
}
...
func isApplied(db *gorm.DB, version string) (bool, error) {
	var count int64
	err := db.Raw("SELECT count(*) FROM user_schema_migrations WHERE version = ?", version).Scan(&count).Error
	return count > 0, err
}

func latestAppliedVersion(db *gorm.DB) (string, error) {
	var version string
	err := db.Raw("SELECT version FROM user_schema_migrations ORDER BY version DESC LIMIT 1").Scan(&version).Error
	return version, err
}
